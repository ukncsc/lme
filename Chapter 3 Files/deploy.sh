...
function generateelasticcert() {
  ...
  echo "subjectAltName = DNS:elasticsearch, IP:127.0.0.1, DNS:"$logstashcn", IP:" $logstaship >>certs/elasticsearch.cnf
...
function generatekibanacert() {
  ...
  echo "subjectAltName = DNS:kibana, IP:127.0.0.1, DNS:"$logstashcn", IP:" $logstaship >>certs/kibana.cnf
...
if [ "$1" == "" ]; then
  ...
  echo "Usage:    ./deploy.sh (install/uninstall/update/upgrade/renew)"
